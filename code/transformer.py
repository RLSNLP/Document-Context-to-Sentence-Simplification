...
class TransformerLayer(nn.Module):
    ...
    def __init__(self, embed_dim, ff_embed_dim, num_heads, dropout, with_external=False, weights_dropout=True):
        ...
        self.self_attn = MultiheadAttention(embed_dim, num_heads, dropout, weights_dropout) # The embed_dim here is corresponding to dim_x.
    ...
    def forward(self, x, kv=None,
                ...
                need_weights=False, dx=None, d_padding_mask=None):

        # x: seq_len x bsz x embed_dim

        residual = x

        if kv is None: # The first used Multi-Head.
        ...
        else: # The second used Multi-Head.
        ...
        if dx is not None:
            ...
            x, external_attn = self.external_attn(query=x, key=dx, value=dx,
                                                  key_padding_mask=d_padding_mask, need_weights=need_weights)
        ...
        if self.with_external:
            ...
            # The external memories is the output of the Encoder.
        ...
        # The feed-forward network used in the Encoder.

        x = gelu(self.fc1(x)) # The gelu activation function which is better than the relu function.
