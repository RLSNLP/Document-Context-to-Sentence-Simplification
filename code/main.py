...
def init_modules():
    ...
    options["is_unicode"] = TRAINING_DATASET_CLS.IS_UNICODE  # True Chinese dataset

    options["has_y"] = TRAINING_DATASET_CLS.HAS_Y

    options["has_context"] = TRAINING_DATASET_CLS.HAS_CONTEXT
    ...
    consts["len_context"] = cfg.MAX_LEN_CONTEXT # maximum length of the context

    consts["batch_size"] = 8 if options["is_debugging"] else TRAINING_DATASET_CLS.BATCH_SIZE
    ...
    consts["max_epoch"] = 51 if options["is_debugging"] else 51
    ...
    [_, dic, hfw, w2i, i2w, w2w] = pickle.load(open(cfg.cc.TRAINING_DATA_PATH + "dic.pkl", "rb")) # The unk signal is corresponding to 1.
    ...
    consts["pad_token_idx"] = modules["w2i"][cfg.W_PAD] # The pad signal is corresponding to 0.
...
def beam_decode(fname, batch, model, modules, consts, options):
    ...
    if options["copy"]:
        ...
        x, x_mask, word_emb, padding_mask, y, len_y, ref_sents, max_ext_len, oovs, dx, d_padding_mask = batch
    ...
    for step in range(consts["max_len_predict"]):
        ...
        if options["copy"]:
            ...
            y_pred, attn_dist = model.decode(ys, tile_x_mask, None, tile_word_emb, tile_padding_mask, tile_x,
                                             max_ext_len, dx, d_padding_mask)
    ...
    write_summ("".join((cfg.cc.BEAM_SUMM_PATH, fname)), sorted_samples, num_samples, options, modules["i2w"], oovs,
               sorted_scores)
...
def predict(model, modules, consts, options):
    ...
    for idx_batch in range(num_batches):
        ...
        dx, d_padding_mask = model.additional_encoder(torch.LongTensor(batch.batch_context).to(options["device"]))

        word_emb, padding_mask = model.encode(torch.LongTensor(batch.x).to(options["device"]), dx, d_padding_mask)
 ...
 \
                              batch.max_ext_len, batch.x_ext_words[idx_s], dx, d_padding_mask)

                else:

                    inputx = (torch.LongTensor(batch.x[:, idx_s]).to(options["device"]), word_emb[:, idx_s, :],
                              padding_mask[:, idx_s], \
 \
                              batch.y[:, idx_s], [batch.len_y[idx_s]], batch.original_summarys[idx_s])
...
def run(existing_model_name=None):
    ...
    if training_model:
        ...
        if options["is_debugging"]:
            ...
            xy_list = pickle.load(open(cfg.cc.TRAINING_DATA_PATH + "document_train.pkl", "rb"))

        batch_list, num_files, num_batches = datar.batched(len(xy_list), options, consts)

        print("document_num_files = ", num_files, ", num_batches = ", num_batches)

        new_xy_list = pickle.load(open(cfg.cc.TRAINING_DATA_PATH + "sentence_train.pkl", "rb"))

        new_batch_list, new_num_files, new_num_batches = newdata.batched(len(new_xy_list), options, consts)

        print("sentence_num_files = ", new_num_files, ", num_batches = ", new_num_batches)
    ...
    if True:  # TODO: refactor
        ...
        if need_load_model:
            ...
            if existing_model_name == None:
                existing_model_name = "s2s.transformer.gpu0.epoch50.2"
        ...
        if training_model:
            ...
            for epoch in range(0, consts["max_epoch"]):
                ...
                batch_list, num_files, num_batches = datar.batched(len(xy_list), options, consts) # The num_files refers to the number of sentences.
 ...
 \
                                         batch.max_ext_len, torch.LongTensor(batch.batch_context).to(options["device"]),

                                         torch.LongTensor(batch.batch_downstairs).to(options["device"]), True)
                ...
                print("Document-level training OK!")

                new_batch_list, new_num_files, new_num_batches = newdata.batched(len(new_xy_list), options,
                                                                       consts)

                for idx_batch in range(new_num_batches):

                    train_idx = new_batch_list[idx_batch]

                    batch_raw = [new_xy_list[xy_idx] for xy_idx in train_idx]
                    ...
                    batch = newdata.get_data(batch_raw, modules, consts, options)
 ...
 \
                                         torch.LongTensor(batch.y_ext).to(options["device"]), \
 \
                                         batch.max_ext_len, None, None, False
 ...
 \
                ...
                if last_total_error > total_error or options["is_debugging"]:
                    ...
                    if not options["is_debugging"]:
                        ...
                        file_name = model_name  + ".epoch" + str(
                            epoch // consts["save_epoch"] + existing_epoch) + "." + str(num_partial)
            ...
            file_name = model_name + ".final.gpu" + str(consts["idx_gpu"]) + ".epoch" + str(
                epoch // consts["save_epoch"] + existing_epoch) + "." + str(num_partial)
