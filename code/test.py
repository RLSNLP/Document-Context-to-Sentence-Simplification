...
def init_modules():
    ...
    consts["len_context"] = cfg.MAX_LEN_CONTEXT # The maximum length of the document context.
...
def beam_decode(fname, batch, model, modules, consts, options):
    ...
    d_padding_mask = d_padding_mask.unsqueeze(1)
    ...
    for step in range(consts["max_len_predict"]):
        ...
        tile_dx = dx.repeat(1, num_live, 1)

        tile_d_padding_mask = d_padding_mask.repeat(1, num_live)
        ...
        if options["copy"]:
            ...
            y_pred, attn_dist = model.decode(ys, tile_x_mask, None, tile_word_emb, tile_padding_mask, x_ext=tile_x,
                                             max_ext_len=max_ext_len, dx=tile_dx, d_padding_mask=tile_d_padding_mask)
        ...
        cand_y_scores = last_scores + y_pred  # larger is better
...
def predict(model, modules, consts, options):
    ...
    for idx_batch in range(num_batches):
        ...
        dx, dc_padding_mask = model.additional_encoder_up(torch.LongTensor(batch.batch_context).to(options["device"]))

        dy, dy_padding_mask = model.additional_encoder_below(torch.LongTensor(batch.batch_downstairs).to(options["device"]))
 ...
 \
                              batch.max_ext_len, batch.x_ext_words[idx_s], dx[:, idx_s, :], d_padding_mask[:, idx_s])
 ...
 \
        ...
        if partial_num >= consts["testing_print_size"]:
            print(total_num, "simplified sentences are generated")
...
def run(existing_model_name=None):
    ...
    if True:  # TODO: refactor
        ...
        if need_load_model:
            ...
            if existing_model_name == None:
                existing_model_name = "s2s.transformer.epoch2.1"
        ...
        if training_model:
            ...
            for epoch in range(0, consts["max_epoch"]):
                ...
                batch_list, num_files, num_batches = datar.batched(len(xy_list), options, consts) # The num_files represents the number of sentences.
 ...
 \
                        ...
                        if not options["is_debugging"]:
                            ...
                            file_name = model_name + ".gpu" + str(consts["idx_gpu"]) + ".epoch" + str(
