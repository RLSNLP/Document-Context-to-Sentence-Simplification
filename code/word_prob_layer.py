...
class WordProbLayer(nn.Module):
    ...
    def forward(self, h, y_emb=None, memory=None, mask_x=None, x_ext=None, max_ext_len=None):

        if self.copy:
            
            atts, dists = self.external_attn(query=h, key=memory, value=memory, key_padding_mask=mask_x,
                                             need_weights=True)
            ...
            x_ext = x_ext.transpose(0, 1).unsqueeze(0).repeat(pred.size(0), 1, 1)

            pred = (g * pred).scatter_add(2, x_ext, (1 - g) * dists)
        ...
        return pred, dists # return the final weights of the predicted output and attention
