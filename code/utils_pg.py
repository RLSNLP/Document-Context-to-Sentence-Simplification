...
def write_for_rouge(fname, ref_sents, dec_words, cfg): # fname is 1, 2, 3, 4, etc.
    dec_sents = []
    ...
    while len(dec_words) > 0:
        ...
        dec_sents.append(' '.join(sent)) # store every sentence

    ref_file = "".join((cfg.cc.GROUND_TRUTH_PATH, fname)) # ground_truth

    decoded_file = "".join((cfg.cc.SUMM_PATH, fname)) # simplified
