...
def get_xy_tuple(downstair, context, cont, head, cfg):
    x = read_cont(cont, cfg)  # original sentence

    y = read_head(head, cfg)  # simplified sentence

    z = read_context(context, cfg)  # preceding sentences

    a = read_downstair(downstair, cfg)  # following sentences
...
# The sentence order in the .txt file should be the simplified sentence, the original sentence, the preceding sentences and the following sentences.
...
def load_lines(d_path, f_name, configs):
    ...
    with open(f_path, 'r') as f:
        ...
        for line in f:
            ...
            fs = line.split("<split>")  # use signal to split

            if len(fs) == 4:

                xy_tuple = get_xy_tuple(fs[3], fs[2], fs[1], fs[0], configs)
...
def to_dict(xys, dic):
    ...
    for xy in xys:
        ...
        sents, summs, context, downstair = xy
...
def read_cont(f_cont, cfg):
    ...
    lines += [cfg.W_EOS]  # add EOS signal in the end of the sentence

    return (lines, f_cont) if len(lines) >= cfg.MIN_LEN_X and len(lines) <= cfg.MAX_LEN_X + 1 else None

def read_context(f_context, cfg):
    lines = []

    line = f_context  # del_num(f_cont)

    words = line.split()

    num_words = len(words)

    if num_words >= cfg.MIN_LEN_CONTEXT and num_words < cfg.MAX_LEN_CONTEXT:

        lines += words

    elif num_words >= cfg.MAX_LEN_CONTEXT:

        lines += words[0:cfg.MAX_LEN_CONTEXT]

    lines += [cfg.W_EOS]  # add EOS signal in the end of the sentence

    return (lines, f_context)


def read_downstair(f_downstair, cfg):
    lines = []

    line = f_downstair  # del_num(f_cont)
...
def prepare_deepmind(d_path):
    ...
    train_xy_list = load_lines(d_path, "context_downstairs_train.txt", configs)

    print("dump train...")

    pickle.dump(train_xy_list, open(TRAINING_PATH + "document_train.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
    ...
    all_dic1, dic_list = load_dict(d_path, "vocab.txt", all_dic1, dic_list)  # The all_dic1 is the dictionary while the dic_list is the vocabulary of all_dic1.

    all_dic2 = to_dict(train_xy_list, all_dic2)  # The vocabulary appearing in the training set forms the dictionary.
    ...
    for w in [configs.W_PAD, configs.W_UNK, configs.W_BOS, configs.W_EOS]:
        ...
        w2i[w] = len(dic)  # The UNK signal is corresponding to 1.
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--data", default="document-simplification", help="dataset path", )

    args = parser.parse_args()

    data_type = "document-simplification"
